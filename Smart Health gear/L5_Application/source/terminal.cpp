...
terminalTask::terminalTask(uint8_t priority) :
        scheduler_task("terminal", 2048, priority),
...
bool terminalTask::taskEntry()
{
    ...
    cp.addHandler(conProHandler,  "con", "Consumer producer board orientation task communication");
	cp.addHandler(spi0Handler,  "spi", "Start spi and flash communication");
	cp.addHandler(binSemaHandler,  "bin", "Task run on interrupt using binary semaphore");
	cp.addHandler(uart2Handler,  "uart2", "Start sending and receiving bytes using UART communication");
	cp.addHandler(orientationCmd,  "ori", "Two options: 'orientation on' or 'orientation off'");
	//cp.addHandler(uart2intHandler,  "uint", "Start sending and receiving bytes using UART interrupt communication");
    // File I/O handlers:
	cp.addHandler(fatHandler,  "fat", "Read the FAT details");
  ...
  //  cp.addHandler(i2cTaskHandler,     "mas",    "I2C master task");
    cp.addHandler(i2cTaskHandlerTrial,     "mas",    "I2C master task");
    cp.addHandler(smartHealthHandler,     "start",    "Display my health details");
