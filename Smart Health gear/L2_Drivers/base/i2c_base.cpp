...
void I2C_Base:: initSlave()
{

}

void I2C_Base:: display()
{
	u0_dbg_printf("LSB is:%x \n", mtempLSB);
	u0_dbg_printf("MSB is:%x \n", mtempMSB);
	//printf("LSB is:%x \n", mtempLSB);
	//printf("MSB is:%x \n", mtempMSB);

}

void I2C_Base:: calcTemperature()
{
	float temperature = 0;
//	u0_dbg_printf("MSB is:%x \n", mtempMSB);
//	mtempMSB = 0x19;
//	mtempLSB = 0x5a;
     for(int i=0; i<8; i++)
     {
    	 if(mtempMSB & (1<<i))
    	 {
    		 ...
    		 temperature += pow(2, i);
    		// u0_dbg_printf("temp m is:%f and i is %d \n", temperature, i);
    	 }
    	 if(mtempLSB & (1<<i))
    	 {

    		 temperature += (1/(pow(2, 8-i)));
    		// u0_dbg_printf("temp l is:%f and i is %d \n", temperature, i);
    	 }
     }

     u0_dbg_printf("Body temperature: %f 'C\n", temperature);
     if(temperature > 38){
...
{
    uint8_t byte = 0;
    readRegisters(deviceAddress, registerAddress, &byte, 1); // j 1 to 2
  //  display();
  //  calcTemperature();
    return byte;
}
...
bool I2C_Base::init(uint32_t pclk, uint32_t busRateInKhz)
{
 ...
 //   const uint32_t half_clock_divider = (pclk / freq_hz) /2;
    const uint32_t half_clock_divider = (pclk / freq_hz);
    u0_dbg_printf("%x freq:", freq_hz);
    u0_dbg_printf("\n %x pclk:", pclk);
    mpI2CRegs->I2SCLH = (half_clock_divider * percent_high) / 100;
    mpI2CRegs->I2SCLL = (half_clock_divider * percent_low ) / 100;

    u0_dbg_printf("\n %x sclh:", mpI2CRegs->I2SCLH);
    u0_dbg_printf("\n %x scll:", mpI2CRegs->I2SCLL);
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    ...
    #define setStop()           clearSTARTFlag();                           \
                                ...
                                if(I2C_READ_MODE(mTransaction.slaveAddr))   \
                                    ...
                                    state = writeComplete;
  //  u0_dbg_printf("%x state\n",mpI2CRegs->I2STAT);
  //  u0_dbg_printf("%x data\n",mpI2CRegs->I2DAT);
    if(mpI2CRegs->I2STAT == 0x50)
    {
    	// u0_dbg_printf("MSB");
    	 mtempMSB = mpI2CRegs->I2DAT; // j
    	// u0_dbg_printf("%x msb:", mtempMSB);
    }
    else if(mpI2CRegs->I2STAT == 0x58)
    {
    	// u0_dbg_printf("LSB");
    	 mtempLSB = mpI2CRegs->I2DAT; // j
    	// u0_dbg_printf("%x lsb:", mtempLSB);
    }
    else
    {
    ...
    switch (mpI2CRegs->I2STAT)
    {
        ...
        case slaveAddressAcked:
        	//u0_dbg_printf("slave ack: slaveAddressAcked \n");
            clearSTARTFlag();
        ...
        case dataAckedBySlave:
        	//u0_dbg_printf("slave ack:dataAckedBySlave \n");
            if (I2C_READ_MODE(mTransaction.slaveAddr)) {
