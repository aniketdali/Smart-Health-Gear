...
bool I2C1::init(unsigned int speedInKhz)
{
     ...
     */
    const uint32_t i2c_pin_mask = ( (1<<0) | (1<<1) ); // j from 10 11
    ...
    LPC_PINCON->PINMODE0 |=  (0xA << 0); // Disable both pull-up and pull-down // j from 20 to 0
   ...
   // LPC_PINCON->PINSEL0 |=  (0xA << 20);  // Enable I2C Pins: SDA, SCL
    ...
    lpc_pclk(pclk_i2c1, clkdiv_8); // from 2 to 1
...
I2C1::I2C1() : I2C_Base((LPC_I2C_TypeDef*) LPC_I2C1_BASE) // changed from 2 to 1
{
       printf("inside 1 construc\n");
}
