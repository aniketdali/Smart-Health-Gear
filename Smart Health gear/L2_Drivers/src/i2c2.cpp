...
bool I2C2::init(unsigned int speedInKhz)
{
     ...
     */
    const uint32_t i2c_pin_mask = ( (1<<10) | (1<<11) ); // j from 10 11 to 0 1
    const bool i2c_wires_are_pulled_high = (i2c_pin_mask == (LPC_GPIO0->FIOPIN & i2c_pin_mask) );

    LPC_PINCON->PINMODE0 &= ~(0xF << 0); // Both pins with Pull-Up Enabled
    LPC_PINCON->PINMODE0 |=  (0xA << 20); // Disable both pull-up and pull-down // j from 20 to 0
  ...
  //  LPC_PINCON->PINSEL0 |=  (0xF << 0); // j

    lpc_pclk(pclk_i2c2, clkdiv_8); // from 2 to 1
...
I2C2::I2C2() : I2C_Base((LPC_I2C_TypeDef*) LPC_I2C2_BASE) // changed from 2 to 1
{
	printf("inside 2 construc\n");
}
